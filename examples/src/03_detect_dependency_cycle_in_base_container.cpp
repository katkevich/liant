...
struct Type1 : Interface1 {
    Type1(liant::ContainerView<Interface2> di)
        : di(di) {}
...
int main() {
    ...
    auto container = liant::makeContainer( //
        liant::baseContainer(container1),  //
        liant::registerInstanceOf<Type3>().as<Interface3>());

    // clang: error: static assertion failed due to requirement '!TypeList<example::Interface2, example::Interface1>::contains()': Detected cycle in your dependencies
